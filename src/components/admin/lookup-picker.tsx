...
export function AdminLookupPicker({
  type,
  label,
  placeholder,
  description,
  ...
  description?: string;
  name?: string;
  initialSelection?: AdminLookupOption | null;
  onSelect?: (option: AdminLookupOption) => void;
  selectionBehavior?: "persist" | "clear";
}) {
  const inputId = useId();
  const [query, setQuery] = useState(initialSelection?.label ?? "");
  const deferredQuery = useDeferredValue(query);
  const [results, setResults] = useState<AdminLookupOption[]>([]);
  const [selected, setSelected] = useState<AdminLookupOption | null>(
  ...
  const runSearch = useEffectEvent(async (nextQuery: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/lookup?type=${type}&q=${encodeURIComponent(nextQuery)}`,
        {
          cache: "no-store",
  ...
  function selectItem(option: AdminLookupOption) {
    startTransition(() => {
      onSelect?.(option);
  ...
  return (
    <div className="space-y-3">
      ...
      {selected && selectionBehavior === "persist" ? (
